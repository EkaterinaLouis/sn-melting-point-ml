...
# 图片显示（建议安装 Pillow：pip install pillow）
try:
...
def build_domain_features(X: pd.DataFrame) -> pd.DataFrame:
    x = X[ELEMENTS].values
    x_safe = np.where(x > 0, x, 1.0)  # 仅用于 log 的安全处理
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Sn-based Alloy Melting Point（不做归一化）")
        ...
        grid = ttk.LabelFrame(left, text="输入各元素分数（不做归一化，按原样使用）", padding=10)
        grid.pack(fill="x")
        ...
        # 状态提示（显示当前输入和，不做任何修改）
        self.info_var = tk.StringVar(value="提示：输入将按原样用于预测（不做归一化）。")
        ttk.Label(left, textvariable=self.info_var, foreground="#555").pack(anchor="w", pady=(4,8))
        ...
        if self.metrics:
            ...
            ttk.Label(evalf, text="未找到 outputs/metrics.json；右侧展示预测-真实散点图。", foreground="#666").pack(anchor="w")
    ...
    def fill_example(self):
        ...
        for el, v in example.items():
            ...
            self.entries[el].insert(0, str(v))
        self.update_sum_info()
    ...
    def update_sum_info(self):
        comp = self.read_inputs()
        s = sum(comp.values())
        self.info_var.set(f"当前输入和 = {s:.6f}（不做归一化，仅提示）。")
